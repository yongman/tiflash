    ...
    ColumnFileSchemaPtr & last_schema)
{
    ...
    return std::make_shared<ColumnFileTiny>(schema, rows, bytes, data_page_id, index_infos);
}
    ...
    IndexInfosPtr index_infos)
{
    ...
    if (!index_infos)
        return std::make_shared<ColumnFileTiny>(column_file_schema, rows, bytes, new_cf_id);
    ...
    return std::make_shared<ColumnFileTiny>(column_file_schema, rows, bytes, new_cf_id, new_index_infos);
}
