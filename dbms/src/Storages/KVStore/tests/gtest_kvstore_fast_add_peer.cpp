...
std::vector<CheckpointRegionInfoAndData> RegionKVStoreOldTestFAP::prepareForRestart(FAPTestOpt opt)
{
    auto & global_context = TiFlashTestEnv::getGlobalContext();
    KVStore & kvs = getKVS();
...
TEST_F(RegionKVStoreOldTestFAP, DumpCheckpointError)
try
{
    auto & global_context = TiFlashTestEnv::getGlobalContext();
    uint64_t region_id = 1;
...
TEST_F(RegionKVStoreOldTestFAP, Cancel5_1)
try
{
...
// Test cancel when building segments
TEST_F(RegionKVStoreOldTestFAP, Cancel5_2)
try
{
...
TEST_F(RegionKVStoreOldTestFAP, TableNotFound)
try
{
    auto mock_data = prepareForRestart(FAPTestOpt{})[0];
    RegionPtr kv_region = std::get<1>(mock_data);
